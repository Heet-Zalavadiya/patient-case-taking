...
this is what Member 3 (Backend) POSTs into the database, so do not rename fields here
...
class InterviewService:
    ...
    def __init__(self, session_id: str = "unknown-session", auto_sync_backend: bool = True):
        self.session_id = session_id
        self.auto_sync_backend = auto_sync_backend
        self.triage_notified = False
        self.turns = []  # each item: {"ai_question", "patient_response_text", "input_mode"}
    ...
    def send_message(self, patient_text: str, input_mode: str = "touch"):
        ...
        if self.auto_sync_backend:
            backend_client.post_turn(self.session_id, ai_question, patient_text, input_mode)
    ...
    def generate_structured_history(self) -> dict:
        ...
        if self.auto_sync_backend:
            backend_client.post_history(self.session_id, history)
