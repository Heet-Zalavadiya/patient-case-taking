...
try:
    ...
    # Gemini enhances chat and summaries, but it must not prevent core patient
    # registration, interview saving, or the doctor queue from starting.
    genai = None
import requests
import urllib3
...
async def chat_endpoint(body: ChatRequest):
    ...
    if not GEMINI_SDK_AVAILABLE or not current_key or current_key == "your_key_here":
        return ChatResponse(
...
Respond with ONLY one word: "VALID" if it describes a genuine health complaint, or "INVALID" if it is unrelated to health (e.g. random words, sports, objects, gibberish, testing text, or anything not describing a symptom or medical issue)."""
    ...
    def _call_gemini() -> str:
        ...
        # 2. Fallback to google.generativeai SDK when it is installed.
